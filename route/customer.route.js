            ...
            callbacks: [
                function (req, res) {
                    ...
                    VALIDATOR.CUSTOMER.SING_UP(customer)
                        ...
                        .then(response => {
                                if (response.length) {
                                    throw new Error(resolveError("CUSTOMER.EMAIL_USED", req))
                                }
                            }
                        )
            ...
            callbacks: [
                ...
                function (req, res) {
                    ...
                    VALIDATOR.CUSTOMER.CHANGE_PASSWORD(customer)
                        ...
                        .then(response => {
                                if (!response.length) {
                                    throw new Error(resolveError("CUSTOMER.WRONG_OLD_PASSWORD", req))
                                }
                            }
                        )
            ...
            callbacks: [
                function (req, res) {
                    ...
                    VALIDATOR.CUSTOMER.VALIDATE_EMAIL({email, emailVerificationCode})
                        ...
                        .then(response => {
                                if (!response.length) {
                                    throw new Error(resolveError("CUSTOMER.WRONG_EMAIL_VERIFICATION_CODE", req))
                                }
                            }
                        )
